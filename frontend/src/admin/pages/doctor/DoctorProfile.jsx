...
const DoctorProfile = () => {
  ...
  const { currency} = useContext(AppAdminContext);
  ...
  const updateProfile = async () => {
    try {
      ...
      const {data} = await axios.post(backendUrl + '/api/doctor/actualizar-perfil', updateData,{headers:{dToken}})
      if (data.success) {
    ...
    <div >
      <div className="flex flex-col gap-4 m-5">
        ...
        <div className="flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white">
          ...
          <div>
            <p className="flex items-center gap-1 text-sm font-medium text-neutral-800">Sobre Mi</p>
            <p className="text-sm text-gray-600 mt-1 max-w-[700px]">{profileData.about}</p>
          </div>
          <p className="text-gray-600 font-medium mt-4">Arancel del Turno: <span className="text-gray-800">{currency} {isEdit ? <input type="number" onChange={(e) => setProfileData(prev => ({ ...prev, fees: e.target.value }))} value={profileData.fees} /> : profileData.fees}</span></p>
          ...
          {
            isEdit
            ?<button onClick={updateProfile} className="px-4 py-1 border text-sm rounded-full mt-5 hover:bg-primary hover:text-white transition-all">Guardar</button>
            : <button onClick={() => setIsEdit(true)} className="px-4 py-1 border text-sm rounded-full mt-5 hover:bg-primary hover:text-white transition-all">Editar</button>
          }
