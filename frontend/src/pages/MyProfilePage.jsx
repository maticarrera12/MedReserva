  ...
  const updateUserProfileData = async () => {
    try {
      ...
      const { data } = await axios.post(
        backendUrl + "/api/user/actualizar-perfil",
        formData,
        { headers: { token } }
      );
      if (data.success) {
            ...
            <div className="inline-block relative cursor-pointer">
              ...
              {!image && (
                <FileUploadRoundedIcon
                  className="w-10 absolute bottom-14 right-12 text-gray-500"
                  style={{ fontSize: "40px" }} // Puedes ajustar el tamaño del ícono aquí
                />
              )}{" "}
            </div>
            <input
